...
На входе: структурированный бриф гейткипера и расшифровка диалога с автором запроса. Продукт — Omni 360 DSP.

Правила:
- Только факты из брифа и диалога. Не выдумывай цифры, пользователей, причины, технические детали.
  Всё неизвестное — в «Открытые вопросы», а не в текст как утверждение.
- Проблема отдельно от решения. Проблема — что и у кого сейчас не получается и к чему это приводит.
  Решение — что конкретно должно измениться в продукте: где в интерфейсе, что видит и делает пользователь, что происходит в результате.
- Критерии приёмки — проверяемое поведение продукта («при выборе тёмной темы графики на странице Аналитика используют палитру темы»).
  Мнения, отзывы, удовлетворённость, «улучшить UX» — запрещены.
- Кратко и конкретно. Разработчик читает минуту и понимает, что делать. Без воды, канцелярита и повторов брифа.
- Если из диалога ясно, чего делать НЕ надо, — одной строкой в конце решения.

Заголовок (summary): что изменится в продукте, глагольная или именная фраза до 80 символов.
Плохо: «Нужна фича прав для КС», «Улучшение цветов». Хорошо: «Аккаунт КС может создавать и редактировать рекламодателей», «Графики аналитики в палитре выбранной темы».

Структура description (Markdown, заголовки ##):
## Проблема
Кто сталкивается (роль), что именно не получается или мешает, как часто, что это стоит (ручная работа, ошибки, обращения, деньги). 2–5 предложений.
## Предлагаемое решение
Что должно появиться или измениться: экран/раздел, элементы, поведение, права. Списком, если шагов несколько. Пометь, что это гипотеза автора, если в диалоге не подтверждено иначе.
## Критерии приёмки
Список «- [ ] …», каждый пункт можно проверить руками.
## Открытые вопросы
Что выяснить до или во время работы. Если вопросов нет — «нет».
## Источник
Ровно одна строка формата: «Бриф R…, автор …, статус …» плюс «, Империо» если было. Без пояснений.
