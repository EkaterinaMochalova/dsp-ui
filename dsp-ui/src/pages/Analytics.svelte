      ...
      <table class="breakdown-table">
        <thead>
          <tr>
            ...
            <th>Период</th>
            <th>
              <button on:click={() => toggleSort('budget')}>
                Плановый бюджет{sortCol === 'budget' ? (sortDir === 1 ? ' ↓' : ' ↑') : ''}
              </button>
            </th>
            <th>
              <button on:click={() => toggleSort('spent')}>
                Потрачено{sortCol === 'spent' ? (sortDir === 1 ? ' ↓' : ' ↑') : ''}
              </button>
            </th>
            <th>
              <button on:click={() => toggleSort('spendPct')}>
                % освоения{sortCol === 'spendPct' ? (sortDir === 1 ? ' ↓' : ' ↑') : ''}
              </button>
            </th>
            <th>
              <button on:click={() => toggleSort('impressions')}>
                Показы{sortCol === 'impressions' ? (sortDir === 1 ? ' ↓' : ' ↑') : ''}
              </button>
            </th>
            <th>
              <button on:click={() => toggleSort('ots')}>
                OTS{sortCol === 'ots' ? (sortDir === 1 ? ' ↓' : ' ↑') : ''}
              </button>
            </th>
            <th>
              <button on:click={() => toggleSort('cpm')}>
                CPM, ₽{sortCol === 'cpm' ? (sortDir === 1 ? ' ↓' : ' ↑') : ''}
              </button>
            </th>
            <th>Статус</th>
          </tr>
        ...
        <tbody>
          {#if statsLoading && !tableRows.length}
            <tr>
              <td colspan="9" class="td-loading">загрузка…</td>
            </tr>
          {:else}
            {#each tableRows as c}
              {@const spent      = c._stats.totalBudgetShowed ?? 0}
              {@const impressions = c._stats.totalShowed ?? 0}
              {@const ots        = c._stats.totalOts ?? 0}
              {@const cpm        = c._stats.cpm ?? 0}
              {@const pct        = c.budget > 0 ? Math.min(100, (spent / c.budget) * 100) : 0}
              <tr class="tr-row">
                <!-- Campaign name + badges -->
                <td class="td-name">
                  <a href="#/campaigns/{c.id}" class="td-name-link">{c.name ?? '—'}</a>
                  <div class="td-badges">
                    <span class="badge-state" style="color:{stateColor(c.state)}">
                      {STATE_LABEL[c.state] ?? c.state}
                    </span>
                    <span class="badge-type" style="background:{TYPE_COLOR[c.type] ?? '#e5e7eb'}">
                ...
                <!-- Planned budget -->
                <td class="td-money">{formatMoney(c.budget ?? 0)}</td>

                <!-- Spent + mini progress bar -->
                <td class="td-spent">
                  {#if statsLoading && !statsMap[c.id]}
                    <span class="td-loading-inline">…</span>
                  {:else}
                    <div class="td-spent-val">{formatMoney(spent)}</div>
                    <div class="mini-bar">
                      <div class="mini-bar-fill" style="width:{pct.toFixed(1)}%"></div>
                    </div>
                  {/if}
                </td>

                <!-- % spend -->
                <td class="td-pct {pct >= 90 ? 'pct-high' : pct >= 50 ? 'pct-mid' : ''}">
                  {#if statsLoading && !statsMap[c.id]}…{:else}{pct.toFixed(1)}%{/if}
                </td>

                <!-- Impressions -->
                <td class="td-num">
                  {#if statsLoading && !statsMap[c.id]}…{:else}{fmt(impressions)}{/if}
                </td>

                <!-- OTS -->
                <td class="td-num">
                  {#if statsLoading && !statsMap[c.id]}…{:else}{fmt(ots)}{/if}
                </td>

                <!-- CPM -->
                <td class="td-money">
                  {#if statsLoading && !statsMap[c.id]}…{:else}{formatMoney(cpm)}{/if}
                </td>

                <!-- Status dot + label -->
                <td class="td-status">
                  <span class="status-dot" style="background:{stateColor(c.state)}"></span>
                  <span class="status-label">{STATE_LABEL[c.state] ?? c.state}</span>
                </td>
              </tr>
            {/each}
            {#if statsLoading}
              <tr>
                <td colspan="9" class="td-stats-loading">
